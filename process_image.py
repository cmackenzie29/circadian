...
def proc_img(img_path, save_path, color_target_R, color_target_G, color_target_B, T):
	...
	for col in range(img.shape[1]):
		pixels = np.arange(img.shape[0])[(img[:,col,0] >= color_target_R[0]) & (img[:,col,0] <= color_target_R[1]) & (img[:,col,1] >= color_target_G[0]) & (img[:,col,1] <= color_target_G[1]) & (img[:,col,2] >= color_target_B[0]) & (img[:,col,2] <= color_target_B[1])]
		if len(pixels) > 0:
